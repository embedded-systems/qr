...
void process(IOBase &io, BitFile &file, int chainpos)
{
  ...
  //shortcut for in2305 lab course
  jtag.setDeviceIRLength(0,6);
  jtag.setDeviceIRLength(1,8);
  jtag.selectDevice(chainpos);
  if(chainpos == 0) programXC3S(jtag,io,file);  
  else programXCF(jtag,io,file);
  
  /* // Synchronise database with chain of devices.
  DeviceDB db(DEVICEDB);
  for(int i=0; i<num; i++){
    int length=db.loadDevice(jtag.getDeviceID(i));
    printf("got length %d for i %d\n", length, i);
